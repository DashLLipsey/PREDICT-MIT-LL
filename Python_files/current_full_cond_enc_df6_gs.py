...
#                              'thresh50', 'thresh100']
allowed_bin_prefixes = [ 'bin2_', 'bin5_', 'bin50_', 'bin500_', ] #'bin0_5_',
allowed_threshold_suffixes = ['thresh_zero', 'thresh0_001', 'thresh0_005', 'thresh0_01', 'thresh0_05', 
                             'thresh0_1', 'thresh0_5', 'thresh1', 'thresh2', 'thresh5', 'thresh10', 
                             'thresh50', 'thresh100']
...
for i, dataset_name in enumerate(sorted(dataset_names), 1):
    ...
    try:
        ...
        # NEW CODE - Create tensors using the new function with filtered Morgan fingerprints:
        # Create tensors first
        print("Creating tensors with filtered Morgan fingerprints...")
        x_train_with_ext, y_train_emb, y_train_tox, y_train_morgan, y_train_filtered_morgan, train_indices_tensor = fd.create_dataset_tensors_condenc_full2_filtered(
                train_data_processed, name_smiles_embedding_df, morgan_df, filtered_morgan_df, device, start_idx=1, stop_idx=-6)

        x_val_with_ext, y_val_emb, y_val_tox, y_val_morgan, y_val_filtered_morgan, val_indices_tensor = fd.create_dataset_tensors_condenc_full2_filtered(
        ...
        # Create model with correct input size using the new filtered encoder
        cond_encoder_current = fd.Cond_Encoder_full_filtered(input_size=actual_input_size,
                                                             output_size=output_size, 
                                                             num_layers=num_layers).to(device)
        
        # Continue with DataLoader creation using the new filtered tensor structure...
        ...
        chemnet_tox_morgan_config = {
            ...
            'wandb_name': f"cond_enc_filtered_super_test_df6_{dataset_name}",
            'gpu': True,
            'encoder_type': "Conditional Encoder ChemNet + Toxicity + Morgan + Filtered Morgan + Group + CE_clean (Super Test)",
        ...
        trained_cond_encoder = fd.train_model_condenc_full2_filtered(
        ...
        if len(super_test_df) > 0:
            ...
            if missing_groups_in_training or missing_ce_clean_in_training:
                ...
                # Recreate tensors with filtered Morgan fingerprints
            ...
            # Create tensors for super test set with filtered Morgan fingerprints
            ...
            # Create super test output DataFrame with new structure (ChemNet + Toxicity + Morgan + Filtered Morgan) + metadata
            emb_cols = [f'cond_emb_{j}' for j in range(512)]  # ChemNet embedding dimensions
            morgan_cols = [f'cond_morgan_{j}' for j in range(regular_morgan_bits)]  # Morgan fingerprint dimensions
            filtered_morgan_cols = [f'cond_filtered_morgan_{j}' for j in range(filtered_morgan_bits)]  # Filtered Morgan fingerprint dimensions
        ...
        del y_val_emb, y_val_tox, y_val_morgan, y_val_filtered_morgan
...
print(f"\n=== CONDITIONAL ENCODER (ChemNet + Toxicity + Morgan + Filtered Morgan + Group + CE_clean) SUPER TEST EVALUATION COMPLETED ===")
...
print(f"Model architecture: {regular_morgan_bits} regular Morgan + {filtered_morgan_bits} filtered Morgan bits")
