...
# Weighted loss function that applies different weights to the loss based on the true LD50 value
def weighted_loss(y_pred, y_true, alpha1=4, alpha2=3, alpha3=2, alpha4=1):
    ...
    # Weight step function based on y_true ranges
    weights = torch.where(
        y_true <= T1,
        torch.full_like(y_true, alpha1),  # First step (y_true <= ln(5))
        torch.where(
            y_true <= T2,
            torch.full_like(y_true, alpha2),  # Second step (ln(5) < y_true <= ln(50))
            torch.where(
                y_true <= T3,
                torch.full_like(y_true, alpha3),  # Third step (ln(50) < y_true <= ln(100))
                torch.full_like(y_true, alpha4)   # Fourth step (y_true > ln(100))
            )
        )
    )
    # Apply weights to squared error loss to get weighted MSE loss
    return (weights * base_loss).mean()


### ======================================= REGRESSION ENCODERS ======================================= ###
# All of the encoder architectures and training functions below are built for regression toxicity prediction
# Or they are built to predict a specific embedding from spectra data without using external conditions
...
# ChemNet Encoder - To predict chemnet embedding from spectra without any other conditioning
# output_size = 512

def train_model_chemnet_encoder(model, train_data, val_data, epochs, learning_rate, criterion, device, config):
...
# Conditional encoder (ChemNet + Toxicity) - First conditional encoder uses only chemnet and tox
# output_size = 513

# Smaller conditional encoder architecture
class Cond_Encoder_12(nn.Module): 
    def __init__(self, input_size, output_size, num_layers):
        super().__init__()
...
# Training function uses both external conditions (group and collision energy)
def train_model_condenc_12e1e2(model, train_data, val_data, epochs, learning_rate, criterion1, criterion2, 
                                    lambda1, lambda2, device, config):
    """
    Training function for conditional encoder 12e1e2 (ChemNet + Toxicity + Group + CE_clean).
    
    NOTE: The batch input includes:
    - Spectra data
    - One-hot encoded Group categories (5 categories)
    - One-hot encoded CE_clean categories (4 categories)
    These external conditions are concatenated in create_dataset_tensors_12e1e2().
    
    Parameters:
    ----------
    lambda1, lambda2 : float
        Weighting factors applied to loss1 and loss2 respectively.
        Final weighted losses are: lambda1 * loss1 and lambda2 * loss2
    """
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    wandb.init(entity=config['wandb_entity'],
               project=config['wandb_project'],
    ...
    for epoch in range(epochs):
        ...
        for batch, true_embeddings, true_log_tox, _ in train_data:
            batch = batch.to(device)  # Includes spectra + Group one-hot + CE_clean one-hot
            true_embeddings = true_embeddings.to(device)
            ...
            # Loss function with modular weights
 ...
 # Uses a sigmoid layer in order to reign in outlier predictions, that sigmoid can be used on other conditions as well   
class Cond_Encoder_123(nn.Module): 
    def __init__(self, input_size, output_size, num_layers):
        super().__init__()
        layers = []
        layer_sizes = np.linspace(input_size, output_size, num_layers + 1, dtype=int)
    ...
    def forward(self, x):
        ...
        # embedding_output = torch.sigmoid(embedding_output) * 3 # Place a similar sigmoid bound on embedding
        # morgan_output = torch.sigmoid(morgan_output) * 2 # Place a similar sigmoid bound on morgan
        # toxicity_output = toxicity_raw # WITHOUT BOUNDING
        # Concatenate back together
        final_output = torch.cat([embedding_output, toxicity_output, morgan_output], dim=1)
        return final_output

### Again, the internal conditions and both group and collision energy as external conditions
def train_model_condenc_123e1e2(model, train_data, val_data, epochs, learning_rate, criterion1, criterion2, criterion3, 
                                           lambda1, lambda2, lambda3, device, config ):
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    wandb.init(entity=config['wandb_entity'],
               project=config['wandb_project'],
               config=config) 
...
#  1234 Conditional encoder - NO SIGMOID CURVE
class Cond_Encoder_1234_nosig(nn.Module):
    def __init__(self, input_size, output_size, num_layers):
        super().__init__()
        layers = []
        layer_sizes = np.linspace(input_size, output_size, num_layers + 1, dtype=int)
    ...
    def forward(self, x):
        ...
        embedding_output = output[:, :512]    # ChemNet embeddings (no activation)
        toxicity_output = output[:, 512:513]     # Raw toxicity output (1 column)
...
# Final conditional encoder with both external conditions, all four internal conditions, and sigmoid activation on
# toxicity
...
# Training functino (uses the weigthed loss function defined at the top)
...
### Same training function as above but without the domain based weighted loss
...
# 2 internal and 2 external conditions 
def create_dataset_tensors_12e1e2(spectra_dataset, embedding_df, device, start_idx=None, stop_idx=None,
                                  all_group_categories=['Q-TOF-positive', 'Q-Orbitrap-positive', 
                                                        'LTQ-Orbitrap-positive', 'QC-GNOMS-Synthetic-Pos', 
                                                        '3DMolMS-Synthetic-Pos'],
                                  all_ce_categories=['NAN', 'low', 'medium', 'high']):
    """
    Create tensors for conditional encoder with group and collision energy information.
    ...
    all_group_categories : list, optional
        List of all possible group categories for one-hot encoding
    all_ce_categories : list, optional
        List of all possible collision energy categories for one-hot encoding
    ...
    # One-hot encode the Group column with all possible categories
    group_categorical = pd.Categorical(spectra_dataset['Group'], categories=all_group_categories)
    group_encoded = pd.get_dummies(group_categorical, prefix='group', dtype=int)
...
# Preprocessing functions to convert input dictionary of spectra into a dataframe, rounding to avoid overly large dfs
...
# Filter spectra into set ranges so that they can be combined after binning is completed. 
...
# This version of the spectra dataframe creation assumes index_id is already created and preserves it
def spectrum_string_to_dataframe(df, spectrum_col, smiles_col):
    ...
    # Collect all unique x values and convert to float
    x_values_set = set()
    data_rows = []
...
# Cate Dunham's smiles to ChemNet embedding code
...
# Binning loop to create full assortment of binned and thresholded datasets for grid search
...
# We have a 2 step process where we first use a conditinoal encoder to  train, using spectra and
# external conditions to predict thre 3 embeddings with the same weighted loss approach as before. 
# Then we can use those embeddings to predict toxicity in a separate model.
...
# Conditional encoder for the three embeddings, excluding toxicity prediction
class Cond_Encoder_134(nn.Module):
    ...
    def forward(self, x):
        ...
        return final_output
# Version of the architectire with a dropout layer to address overfitting
...
def train_model_condenc_134e1e2(model, train_data, val_data, epochs, learning_rate, criterion1, criterion3, criterion4,
                                 lambda1, lambda3, lambda4, device, config):
    # Define optimizer
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    ...
    for epoch in range(epochs):
        ...
        for batch_with_ext, true_embeddings, true_morgan, true_filtered_morgan, _ in train_data:
            ...
            # Morgan Loss
            batch_predicted_morgan = batch_predicted_combined[:, 512:512 + 2048]
            loss3 = criterion3(batch_predicted_morgan, true_morgan)

            # Filtered Morgan Loss
            batch_predicted_filtered_morgan = batch_predicted_combined[:, 512 + 2048:]
            ...
            # Total Loss
            total_loss = weighted_loss1 + weighted_loss3 + weighted_loss4
        ...
        val_filtered_morgan_loss = 0.0
        with torch.no_grad():
            for val_batch_with_ext, val_true_embeddings, val_true_morgan, val_true_filtered_morgan, _ in val_data:
                val_batch_with_ext = val_batch_with_ext.to(device)
                val_true_embeddings = val_true_embeddings.to(device)
                val_true_morgan = val_true_morgan.to(device)
                val_true_filtered_morgan = val_true_filtered_morgan.to(device)
                ...
                # Calculate individual losses
                val_batch_predicted_embeddings = val_batch_predicted[:, :512]
                val_batch_predicted_morgan = val_batch_predicted[:, 512:512 + 2048]
                val_batch_predicted_filtered_morgan = val_batch_predicted[:, 512 + 2048:]
                ...
                val_loss += val_weighted_loss1.item() + val_weighted_loss3.item() + val_weighted_loss4.item()
                val_embedding_loss += val_weighted_loss1.item()
                val_morgan_loss += val_weighted_loss3.item()
                val_filtered_morgan_loss += val_weighted_loss4.item()
...
# Created tensors from the intermediate dataframes 
def create_dataset_tensors_toxicity_classifier_134(embeddings_df, device):
    ...
    tuple
        A tuple containing:
        - concatenated_embeddings_tensor (torch.Tensor): Tensor of concatenated predicted embeddings.
...
# This model uses the listed embeddings to predict toxicity classes
def train_toxicity_classifier_134(model, train_data, val_data, epochs, learning_rate, criterion, device, config):
    """
    Train the toxicity classifier model.

    Parameters:
    ----------
    model : nn.Module
        The toxicity classifier model
    train_data : DataLoader
        Training data loader containing (concatenated_embeddings, toxicity_labels)
    val_data : DataLoader
        Validation data loader containing (concatenated_embeddings, toxicity_labels)
    epochs : int
    ...
    device : torch.device
        Device to train on
    config : dict
        Configuration dictionary for wandb logging
    ...
    for epoch in range(epochs):
        ...
        for concatenated_embeddings, true_tox_class in train_data:
            concatenated_embeddings = concatenated_embeddings.to(device)
            true_tox_class = true_tox_class.to(device)

            optimizer.zero_grad()
            predicted_logits = model(concatenated_embeddings)

            # Calculate loss
            loss = criterion(predicted_logits, true_tox_class)
            ...
            # Calculate accuracy
            _, predicted_classes = torch.max(predicted_logits, 1)
            total_train += true_tox_class.size(0)
            correct_train += (predicted_classes == true_tox_class).sum().item()

        # Calculate average training metrics per epoch
        average_train_loss = running_loss / len(train_data)
        ...
        with torch.no_grad():
            for val_concatenated_embeddings, val_true_tox_class in val_data:
                val_concatenated_embeddings = val_concatenated_embeddings.to(device)
                val_true_tox_class = val_true_tox_class.to(device)

                val_predicted_logits = model(val_concatenated_embeddings)

                # Calculate loss
                loss = criterion(val_predicted_logits, val_true_tox_class)
                val_loss += loss.item()
                
                # Calculate accuracy
                _, predicted_classes = torch.max(val_predicted_logits, 1)
                total_val += val_true_tox_class.size(0)
                correct_val += (predicted_classes == val_true_tox_class).sum().item()
...
# This function trains the direct toxicity prediction model
def train_direct_toxicity_encoder_e1e2(model, train_data, val_data, epochs, learning_rate, criterion, device, config):
    """
    Train the direct toxicity prediction encoder model.

    Parameters:
    ----------
    model : nn.Module
        The direct toxicity encoder model
    train_data : DataLoader
        Training data loader containing (spectra_with_conditions, toxicity_labels)
    val_data : DataLoader
        Validation data loader containing (spectra_with_conditions, toxicity_labels)
    epochs : int
        Number of training epochs
    learning_rate : float
        Learning rate for optimizer
    ...
    for epoch in range(epochs):
        ...
        for spectra_with_conditions, true_tox_class, _ in train_data:
            spectra_with_conditions = spectra_with_conditions.to(device)
            true_tox_class = true_tox_class.to(device)

            optimizer.zero_grad()
            predicted_logits = model(spectra_with_conditions)
        ...
        with torch.no_grad():
            for val_spectra_with_conditions, val_true_tox_class, _ in val_data:
                val_spectra_with_conditions = val_spectra_with_conditions.to(device)
                val_true_tox_class = val_true_tox_class.to(device)

                val_predicted_logits = model(val_spectra_with_conditions)
