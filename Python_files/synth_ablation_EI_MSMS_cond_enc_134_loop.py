...
VAL_INT_DIR  = "/home/dlipsey/MITLincolnLabs/MIT_LL_data/2step_EI_MSMS_synth_abl_Loop_intermediate"
VAL_FINAL_DIR = "/home/dlipsey/MITLincolnLabs/MIT_LL_data/2step_EI_MSMS_synth_abl_Loop"
SUPER_INT_DIR  = "/home/dlipsey/MITLincolnLabs/MIT_LL_data/2step_EI_MSMS_synth_abl_Loop_super_test_intermediate"
SUPER_FINAL_DIR = "/home/dlipsey/MITLincolnLabs/MIT_LL_data/2step_EI_MSMS_synth_abl_loop_super_test"
for d in [VAL_INT_DIR, VAL_FINAL_DIR, SUPER_INT_DIR, SUPER_FINAL_DIR]:
    os.makedirs(d, exist_ok=True)

# Step 1 embedding inputs - UPDATED FOR EI_MSMS DATA
name_smiles_embedding_df = pd.read_parquet("/home/dlipsey/MITLincolnLabs/MIT_LL_data/merged_EI_MSMS_chemnet.parquet")
morgan_df = pd.read_parquet("/home/dlipsey/MITLincolnLabs/MIT_LL_data/merged_EI_MSMS_morganfp.parquet")
filtered_morgan_df = pd.read_parquet("/home/dlipsey/MITLincolnLabs/MIT_LL_data/merged_EI_MSMS_filtered_morganfp.parquet")
...
id_to_group = dict(zip(merged_EI_MSMS_spectra['index_id'], merged_EI_MSMS_spectra['Group']))
id_to_ce_clean = dict(zip(merged_EI_MSMS_spectra['index_id'], merged_EI_MSMS_spectra['CE_clean']))

# Load synthetic flag from merged_EI_MSMS_spectra for all index_ids
id_to_synthetic = dict(zip(merged_EI_MSMS_spectra['index_id'], merged_EI_MSMS_spectra['synthetic'].fillna(0)))
...
for loop_counter in range(num_loops):
    ...
    train_data_processed = fd.add_response_and_log_response(train_data.copy(), merged_EI_MSMS_subset, smiles_col='SMILES_spectra')
    train_data_processed = fd.add_tox_levels(train_data_processed)
    train_data_processed['index'] = train_data_processed['index_id']
    
    test_data_processed = fd.add_response_and_log_response(test_data.copy(), merged_EI_MSMS_subset, smiles_col='SMILES_spectra')
    ...
    embedding_config = {
        ...
        'wandb_name': f"step1_embedding_EI_MSMS_synth_abl_{dataset_name}_loop{loop_counter}",
        'gpu': True,
    ...
    if len(super_test_df) > 0:
        ...
        super_test_df = super_test_df[~super_test_df['index_id'].isin(synthetic_index_ids)].copy()
        ...
        super_test_processed = fd.add_response_and_log_response(super_test_df.copy(), merged_EI_MSMS_subset, smiles_col='SMILES_spectra')
    ...
    tox_config = {
        'wandb_entity': 'dashlipsey-worcester-polytechnic-institute',
        'wandb_project': 'MIT-Lincoln-Lab',
        'wandb_name': f"step2_toxicity_EI_MSMS_synth_abl_{dataset_name}_loop{loop_counter}",
        'gpu': True,
