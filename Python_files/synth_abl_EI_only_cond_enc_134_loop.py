...
# --- Output folders (SYNTHETIC ABLATION) ---
VAL_INT_DIR  = "/home/dlipsey/MITLincolnLabs/MIT_LL_data/2_step_EI_only_synth_abl_loop_intermediate"
VAL_FINAL_DIR = "/home/dlipsey/MITLincolnLabs/MIT_LL_data/2_step_EI_only_synth_abl_loop"
SUPER_INT_DIR  = "/home/dlipsey/MITLincolnLabs/MIT_LL_data/2_step_EI_only_synth_abl_loop_intermediate_super_test"
SUPER_FINAL_DIR = "/home/dlipsey/MITLincolnLabs/MIT_LL_data/2_step_EI_only_synth_abl_loop_super_test"
for d in [VAL_INT_DIR, VAL_FINAL_DIR, SUPER_INT_DIR, SUPER_FINAL_DIR]:
    os.makedirs(d, exist_ok=True)

# Step 1 embedding inputs - UPDATED FOR EI_ONLY DATA
name_smiles_embedding_df = pd.read_parquet("/home/dlipsey/MITLincolnLabs/MIT_LL_data/EI_only_chemnet.parquet")
morgan_df = pd.read_parquet("/home/dlipsey/MITLincolnLabs/MIT_LL_data/EI_only_morganfp.parquet")
filtered_morgan_df = pd.read_parquet("/home/dlipsey/MITLincolnLabs/MIT_LL_data/EI_only_filtered_morganfp.parquet")
...
grid_search_folder = "/home/dlipsey/MITLincolnLabs/MIT_LL_data/EI_only_dataframes"
...
super_test_smiles = [
    ...
    'CCN1CC2(COC)C(OC(C)=O)CC(OC)C34C5CC6(O)C(OC)C(O)C(OC(=O)c5ccccc5)(C5C6OC(=O)c6ccccc6)C(C(OC)C23)C14',
    # Level 1
...
id_to_group = dict(zip(EI_only_spectra['index_id'], EI_only_spectra['Group']))
id_to_ce_clean = dict(zip(EI_only_spectra['index_id'], EI_only_spectra['CE_clean']))

# Load synthetic flag from EI_only_spectra for all index_ids
id_to_synthetic = dict(zip(EI_only_spectra['index_id'], EI_only_spectra['synthetic'].fillna(0)))

# Map SMILES_spectra -> index_id in the major dataset
...
print("SYNTHETIC ABLATION MODE: Synthetic data will be EXCLUDED from training")

for loop_counter in range(num_loops):
    print(f'\n========== LOOP {loop_counter+1}/{num_loops} (SYNTHETIC ABLATION) ==========')
    ...
    # === SYNTHETIC ABLATION: EXCLUDE synthetic data ===
    synthetic_mask = filtered_dataset['index_id'].map(lambda idx: id_to_synthetic.get(idx, 0)==1)
    real_mask = ~synthetic_mask

    # Only use real data for training
    real_data = filtered_dataset[real_mask].copy()
    print(f"Using only real data: {len(real_data)} spectra (excluded {synthetic_mask.sum()} synthetic)")
    ...
    # SYNTHETIC ABLATION: Do NOT add synthetic data to training
    ...
    train_data_processed = fd.add_response_and_log_response(train_data.copy(), EI_only_subset, smiles_col='SMILES_spectra')
    train_data_processed = fd.add_tox_levels(train_data_processed)
    train_data_processed['index'] = train_data_processed['index_id']
    
    test_data_processed = fd.add_response_and_log_response(test_data.copy(), EI_only_subset, smiles_col='SMILES_spectra')
    ...
    embedding_config = {
        ...
        'wandb_name': f"step1_embedding_EI_only_synth_abl_{dataset_name}_loop{loop_counter}",
        'gpu': True,
        'encoder_type': "Step 1: Embedding Model (ChemNet + Morgan + Filtered Morgan + Group + CE_clean) - SYNTHETIC ABLATION",
    ...
    intermediate_df['index_id'] = combined_processed['index_id'].values
    ...
    if len(super_test_df) > 0:
        ...
        super_test_processed = fd.add_response_and_log_response(super_test_df.copy(), EI_only_subset, smiles_col='SMILES_spectra')
        ...
        super_test_emb_df['index_id'] = super_test_processed['index_id'].values
    ...
    tox_config = {
        'wandb_entity': 'dashlipsey-worcester-polytechnic-institute',
        'wandb_project': 'MIT-Lincoln-Lab',
        'wandb_name': f"step2_toxicity_EI_only_synth_abl_{dataset_name}_loop{loop_counter}",
        'gpu': True,
        'model_type': "Step 2: Toxicity Classifier - SYNTHETIC ABLATION",
    ...
    if len(super_test_df) > 0:
        ...
        if os.path.exists(super_interm_path):
            ...
            super_test_intermediate_df['train'] = 0
...
print('-'*50)
print('All loops completed (SYNTHETIC ABLATION).')
